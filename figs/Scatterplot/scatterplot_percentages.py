...
def create_scatter_plot_4panel(jsonl_file, output_file='scatterplot_percentages_4panel.png'):
    ...
    for entry in data:
        ...
        not_addressed_pct = float(entry['not_addressed_percentage'].rstrip('%'))
        
        support_percentages.append(support_pct)
        not_addressed_percentages.append(not_addressed_pct)
    ...
    fig.suptitle('Support Percentage vs Not Addressed Percentage by Concordance Measure', fontsize=16)
    ...
    for idx, (concordance_values, title) in enumerate(concordance_data):
        ...
        support_jitter = [x + np.random.normal(0, 1) for x in support_percentages]
        not_addressed_jitter = [y + np.random.normal(0, 1) for y in not_addressed_percentages]
        
        ax.scatter(support_jitter, not_addressed_jitter, c=colors, alpha=0.7, s=50)
        
        ax.set_xlabel('Support Percentage (%)')
        ax.set_ylabel('Not Addressed Percentage (%)')
        ax.set_title(title)
...
def create_scatter_plot(jsonl_file, output_file='scatterplot_percentages.png'):
    data = load_data(jsonl_file)
    ...
    scatter = plt.scatter(support_jitter, not_addressed_jitter, c=colors, alpha=0.7, s=60)
    
    plt.xlabel('Support Percentage (%)')
    plt.ylabel('Not Addressed Percentage (%)')
    plt.title('Support Percentage vs Not Addressed Percentage by Case')
